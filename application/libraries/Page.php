...
class Page {
	...
	public function __construct() {
		...
		$first = $this->_CI->uri->segment(1);//get controller
		$second	= '';
		
		if ($first == '') {
			$first = $this->_CI->router->class; //get the name of the controller or class
		}
		else {
			$second	= $this->_CI->uri->segment(2); //get method
		}
		...
		$prev = $this->_CI->agent->referrer();//get prev site
		...
		else if ($first == 'Pengguna'){

		}
		else if ($second == 'insert'){

		}
		else {
			if ($this->_CI->session->has_userdata('pengguna')) {
				#...
			}
			else {
				redirect(site_url('/'));
